...
四色缩放大头针。经典大头针轮廓不再承载数字，烧烤使用辣椒橙、粉面使用稻穗金、小吃使用河流蓝、其他使用青菜绿；远景时收小以显露街区，近景时随缩放平滑展开，选中时仅显露双层外环。它同时解决地图识别、地图拥挤与餐饮类别扫读三个问题。地图空白区使用极淡的湘江水纹等高线，不抢内容。
...
| accent | 0.64 0.19 35 | #E65C32 | 唯一品牌强调色；搜索、选中、主按钮；烧烤数据编码 |
| accent-dark | 0.50 0.17 35 | #B83C1E | 强调色文字与 hover |
| success | 0.55 0.13 151 | #238A58 | 营业中；其他数据编码 |
| warning | 0.72 0.15 79 | #D98A16 | 即将打烊；粉面数据编码 |
| danger | 0.56 0.20 25 | #CF3F36 | 错误、已打烊 |
| info | 0.53 0.11 230 | #287C9E | 定位、信息；小吃数据编码 |
